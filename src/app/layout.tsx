import type { Metadata } from 'next';
import { Urbanist, Nunito_Sans, Petit_Formal_Script } from 'next/font/google';
import './globals.css';
import Navbar from '../../components/layout/Navbar';
import Footer from '../../components/layout/Footer';

const urbanist = Urbanist({
  variable: '--font-urbanist',
  subsets: ['latin'],
  display: 'swap',
});

const nunitoSans = Nunito_Sans({
  variable: '--font-nunito-sans',
  subsets: ['latin'],
  weight: ['400', '700'],
  display: 'swap', // 'swap' is recommended to avoid layout shifts
});

const petitFormalScript = Petit_Formal_Script({
  variable: '--font-petit-formal-script',
  subsets: ['latin'],
  weight: '400',
  display: 'swap',
});

export const metadata: Metadata = {
  title: 'Urban Ritual Cafe',
  description:
    "Bay Area's most creative boba & coffee drinks. Handcrafted boba teas, specialty coffee, and seasonal drinks in San Francisco and San Mateo.",
  icons: {
    icon: '/logos/favicon.png',
  },
};

export default function RootLayout({
  children,
}: Readonly<{
  children: React.ReactNode;
}>) {
  return (
    <html lang="en">
      <body
        className={`${urbanist.variable} ${nunitoSans.variable} ${petitFormalScript.variable} antialiased`}
      >
        <Navbar />
        {children}
        <Footer />
      </body>
    </html>
  );
}
